...
interface FriendRepository : JpaRepository<Friend, FriendKey> {
    @Query("SELECT COUNT(f) FROM Friend f WHERE (f.sender = :user OR f.receiver = :user) AND f.state = dev.jombi.ubi.util.state.FriendState.ACCEPTED")
    ...
    @Query("SELECT f FROM Friend f WHERE f.receiver = :user AND f.state = dev.jombi.ubi.util.state.FriendState.PENDING")
